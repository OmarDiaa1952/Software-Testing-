...
exports.withdraw = async (req, res, next) => {
    ...
    const trans = Trans.find({ _id, date, action: 0 });
    const user = await User.findById(_id);
    try {
        if (user) {
            ...
            if (user.balance - amount == 100)
                ...
                const withdrawnToday = (await trans).reduce(
                ...
                if (withdrawnToday > 15000)
                    throw new Error(
                        "Can't with withdraw more than 15,000$ per day"
                    );
                ...
                Promise.all([newTrans.save(), user.save()]).then((result) =>
                    res.json({ amount, oldBalance, balance: user.balance })
                );
            }
